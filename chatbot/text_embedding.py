...
embedding_encoding = "o200k_base"  # gpt-4o-mini
max_tokens = 1500
...
def split_into_many(text, max_tokens=500):
    ...
    for sentence, token in zip(sentences, n_tokens):
        # これまでのトークン数と現在の文のトークン数を合計した値が最大トークン数を超える場合は、
        # チャンクをチャンクのリストに追加し、チャンクとトークン数をリセット
...
for row in df.iterrows():
    ...
    if row[1]['text'] is None:
        continue
    # トークン数が最大トークン数より大きい場合は、テキストを「shortened」リストに追加
